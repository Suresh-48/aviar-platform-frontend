...
import { Container, Row, Col, Form, FormControl, Dropdown, InputGroup, Modal } from "react-bootstrap";
...
// Styles
import "../../css/EditTeacherDetails.css";
...
const BankDetails = Yup.object().shape({
  ...
  confirmAccountNumber: Yup.number()
    ...
    .oneOf([Yup.ref("accountNumber"), null], "Accounts Number is must match")
...
const EditTeacherDetails = (props) => {
  ...
  const fileUploadInputChange = async (e) => {
    ...
    if (type === "image") {
      Api.patch("api/v1/teacher/image/upload", {
        teacherId: teacherId,
        image: base64, // Use the actual base64 from conversion
        userId: userId,
      })
        .then((response) => {
  ...
  const getTeacherDetail = () => {
    ...
    Api.get(`api/v1/teacher/${teacherId}`).then((response) => {
      ...
      // Set React Quill content directly
      setSpecialityDescription(data?.specialityDescription || "");
      setDescriptionValue(data?.specialityDescription || "");
  ...
  const onChangeDescription = ({ setFieldValue }, value) => {
    // Remove HTML tags to get plain text for validation
    const plainText = value.replace(/<[^>]*>/g, '');
    setDescriptionValue(plainText);
    setFieldValue("descriptionValue", plainText);
    setSpecialityDescription(value);
  };

  const onChangeAboutUs = ({ setFieldValue }, value) => {
    // Remove HTML tags to get plain text for validation
    const plainText = value.replace(/<[^>]*>/g, '');
    setAboutUsValue(plainText);
    setFieldValue("aboutUsValue", plainText);
    setAboutUs(value);
  };
  ...
  const submitForm = (values, { resetForm }) => {
    ...
    // Use the HTML content directly from React Quill
    const convertedData = specialityDescription;
    const aboutUsConvertedData = aboutUs;
  ...
  // Safe city options function
  const getCityOptions = () => {
    if (!states[stateCode] || !states[stateCode].cities) {
      return [];
    }
    return states[stateCode].cities.map((item, key) => ({
      label: item,
      value: item,
    }));
  };
  ...
  const submitBankDetails = (values) => {
    ...
    Api.post("/api/v1/teacher/create/customer", {
      accountType: "TEACHER_ACCOUNT_TYPE", // Replace with actual constant
      country: "TEACHER_ACCOUNT_COUNTRY", // Replace with actual constant
      currency: "TEACHER_ACCOUNT_CURRENCY", // Replace with actual constant
          ...
          <div>
            {value === 0 && (
              <Formik
                ...
                enableReinitialize
                validationSchema={teacherDetails.loginType === "Email" ? EmailSignInSchema : GoogleAndFacebookSignInSchema}
                onSubmit={submitForm}
              >
                    ...
                    <Row>
                      <Col sm={4} xs={12} className="px-4 pt-4">
                        <Dropdown className="dropdown-profile-list">
                          <Dropdown.Toggle className="teacher-menu-dropdown p-0" variant="link">
                            <div>
                              <div>
                                {imagePreview ? (
                                  <img
                              ...
                              <div className="d-flex justify-content-center mt-3">
                                <span style={{ fontSize: 11, color: "black" }}>Click Here To Upload Profile</span>
                                <FontAwesomeIcon icon={faPen} size="sm" color="#1d1464" className="mx-1" />
                              </div>
                            </div>
                          </Dropdown.Toggle>
                          <Dropdown.Menu className="profile-dropdown-status py-0">
                      ...
                      <Col xs={12} sm={8} className="px-4">
                        ...
                        <Form.Group className="form-row mb-3">
                          ...
                          <div className="teacher-description">
                            <ReactQuill
                              ...
                              onChange={(value) => onChangeDescription({ setFieldValue }, value)}
                        ...
                        <Form.Group className="form-row mb-3">
                          ...
                          <div className="teacher-description">
                            <ReactQuill
                              value={aboutUs}
                              onChange={(value) => onChangeAboutUs({ setFieldValue }, value)}
                              modules={quillModules}
                              formats={quillFormats}
                              theme="snow"
                            />
