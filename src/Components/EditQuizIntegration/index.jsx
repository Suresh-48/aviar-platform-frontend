...
const EditQuizIntegration = () => {
  ...
  const questionList = () => {
    const userId = localStorage.getItem("userId");
    Api.get(`api/v1/lessonQuiz/${location.state?.quizId}`, {
      params: {
        courseLessonId: lessonId,
        userId: userId,
      },
  ...
  // Show error if quizId is not available
  if (!quizId) {
    return (
      <div className="p-3">
        <div className="alert alert-danger text-center">
          <h5>Error: Quiz information not found</h5>
          <p>Please go back and select a quiz to edit.</p>
          <button 
            onClick={() => navigate(-1)} 
